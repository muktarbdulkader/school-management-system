...
function UintTaskReport({ hideHeadSection, id }) {
  ...
  useEffect(() => {
    const handleFetchingTaskReport = async () => {
      if (selectedYear?.id) {
        ...
        // Build the API URL with manager_id if available
        let Api = `${Backend.api}${Backend.employeeProfileTask}${id}?fiscal_year_id=${selectedYear?.id}`;

        if (manager_id) {
          Api += `&manager_id=${manager_id}`;
        }
        const header = {
