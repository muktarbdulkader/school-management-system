...
import BestPerformerUnits from './BestPerformerUnits';
...
const UnitLeaderboard = () => {
  ...
  const handleGettingLeaderboard = async (reload) => {
    try {
      ...
      const Api = Backend.api + Backend.unitLeaderboard + `?fiscal_year_id=${selectedYear?.id}&order=${filter.order}`;
      const header = {
            ...
            <Grid container>
              ...
              <Grid item xs={12} sm={12} md={6} lg={6} xl={6} sx={{ display: 'flex', justifyContent: 'center' }}>
                {loading ? (
                  ...
                  <BestPerformerUnits unit={data[selectedIndex]} />
                )}
              </Grid>
            </Grid>
          </DrogaCard>
