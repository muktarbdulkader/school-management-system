  ...
  useEffect(() => {
    const handleFetchingPerformance = async () => {
      if (selectedYear?.id) {
        ...
        const Api = Backend.api + Backend.myPerformance + `?fiscal_year_id=${selectedYear?.id}&filter_by=${filter.frequencies}`;
  ...
  return (
    <Grid item xs={12}>
      <Grid container spacing={gridSpacing}>
        <Grid item xs={12} sm={12} md={!hidePerformanceDetail && 6} lg={!hidePerformanceDetail && 4} xl={!hidePerformanceDetail && 4}>
          <DrogaCard sx={{ mt: 2 }}>
                        ...
                        {performance?.map((period, index) => {
                          ...
                          return (
                            <PerformanceCard
                              ...
                              onPress={() =>
                                navigate('/per-kpi-performance', { state: { state: period[periodName], filtered_by: filter.frequencies } })
                              }
                            />
                          );
                        })}
